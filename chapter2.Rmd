...
summary(model_sig)
```
The three different diagnostic plots are generated below.
```{r, fig.width=11, fig.height=9}
par(mfrow = c(2,2))
plot(model_sig, which = c(1,2,5))
```
The assumptions behind all three models is linearity and normality. Based on the above plots, we can conclude that the errors are normally distributed (clearly observed in q-q plot). Moreover, residual versus model showed that the errors are not dependent on the attitude variable.Thus, assumptions in all models are valid.
